...
|----|-------------|----------|--------|--------|
| UNK-039 | Purpose of `shared/execution/provider/` directory | `shared/execution/src/main/java/com/conductor/shared/execution/provider/` | **Resolved** (this run) | `ProviderClient.java` and `WhatsAppProvider.java` discovered; directory classified as `shared/execution-provider` submodule |
...
| UNK-041 | Purpose of `/shared/middleware/tenant/` cannot be distinguished from `/shared/auth/` without reading source | `shared/middleware/src/main/java/com/conductor/shared/middleware/tenant/` | **Open** | Directory structure and class names (`TenantContextPropagator`, `TenantRequestInterceptor`) overlap significantly with `/shared/auth/`'s `TenantSecurityFilter`. Cannot determine which is authoritative for tenant context without reading source. Requires human annotation or source review. |
